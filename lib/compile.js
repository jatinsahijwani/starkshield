...
async function compileCircuit(circuitPath) {
    ...
    const generatedContract = path.join(outDir, baseName, "src", "groth16_verifier.cairo");
   
    console.log(`✅ All files successfully generated in: ${outDir}`);
    console.log(`   → Cairo verifier ready at: ${generatedContract}`);

    const toolFile = path.join(outDir, baseName, ".tool-versions");
    let content = fs.readFileSync(toolFile, 'utf-8');
    content = content.replace('starknet-foundry 0.53.0', 'starknet-foundry 0.57.0');
    fs.writeFileSync(toolFile, content, 'utf8');
    console.log("New file content of .tool-versions", content);
    console.log('Done! starknet-foundry updated to 0.57.0');
